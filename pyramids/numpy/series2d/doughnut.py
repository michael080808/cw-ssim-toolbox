...
class Doughnut(CoordSys):
    def __init__(self, window: Union[int, Tuple[int], Tuple[int, int]], levels: int, transition: float = 1.0, dtype: numpy.dtype = numpy.float64):
        assert levels >= 2
        CoordSys.__init__(self, window, dtype)
        self.levels, self.transition = levels, transition
        self.radius = numpy.sqrt(self.x_axis ** 2 + self.y_axis ** 2)

    def _logarithm_indices(self, _lower: float, _upper: float):
        log_upper = numpy.log(_upper)
        return numpy.clip(-(numpy.log(numpy.where(self.radius <= 0, numpy.finfo(self.radius.dtype).eps, self.radius)) - log_upper) / (numpy.log(_lower) - log_upper), a_min=-1, a_max=+0)
    ...
    def groups(self):
        ...
        for _upper, _lower in itertools.pairwise([max(self.window) // 2 ** level for level in range(1, self.levels + 1)]):
            log_id = self._logarithm_indices_with_transition(_lower, _upper) * numpy.pi / 2
            result.extend([result.pop() * numpy.cos(+log_id), numpy.sin(-log_id)])
        return result
