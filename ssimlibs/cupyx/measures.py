...
from .._dualtree.cupyx import DualTreeComplexWaveletTransform
from .._pyramids.cupyx import SteerablePyramid2D
...
class _MeasureWithMeanComponents(_MeasureWithRealComponents):
    ...
    def luminance(self, _a: cupy.ndarray, _b: cupy.ndarray):
        ...
        return cupy.mean(super().luminance(_a, _b), axis=axis)

    def contrasts(self, _a: cupy.ndarray, _b: cupy.ndarray):
        axis = tuple(range(-self._dim, 0))
        return cupy.mean(super().contrasts(_a, _b), axis=axis)

    def structure(self, _a: cupy.ndarray, _b: cupy.ndarray):
        axis = tuple(range(-self._dim, 0))
        return cupy.mean(super().structure(_a, _b), axis=axis)
...
class _UIQI(ABC, metaclass=ABCMeta):
    ...
    def __call__(self, _a: cupy.ndarray, _b: cupy.ndarray):
        ...
        statistics = self.statistics(_a, _b)
        p_numerator = 4 * statistics['σ(A*B)'] * statistics['μ(A^1)'] * statistics['μ(B^1)'] + cupy.finfo(_a.dtype).eps
        denominator = (statistics['σ(A^2)'] + statistics['σ(B^2)']) * (statistics['μ(A^1)'] ** 2 + statistics['μ(B^1)'] ** 2) + cupy.finfo(_a.dtype).eps
...
class mUIQI(_MeasureWithMeanComponents, _UIQI):
    ...
    def __call__(self, _a: cupy.ndarray, _b: cupy.ndarray):
        return cupy.mean(super().__call__(_a, _b), axis=tuple(range(-self._dim, 0)))
...
class MsSSIM(_MeasureWithRealComponents):
    ...
    def down_sample(self, _t: cupy.ndarray, level: int):
        axis = tuple(range(-self._dim, 0))
        _mid = cupy.fft.fftn(_t + 0.0j, axes=axis)
        _mid = cupy.fft.fftshift(_mid, axes=axis)
        _mid = _mid[self.calc_slices(_mid.shape, level)]
        _mid = cupy.fft.ifftshift(_mid, axes=axis)
        return cupy.fft.ifftn(_mid, axes=axis).real

    def luminance(self, _a: cupy.ndarray, _b: cupy.ndarray, level: int = 0):
        return super().luminance(self.down_sample(_a, level), self.down_sample(_b, level))

    def contrasts(self, _a: cupy.ndarray, _b: cupy.ndarray, level: int = 0):
        return super().contrasts(self.down_sample(_a, level), self.down_sample(_b, level))

    def structure(self, _a: cupy.ndarray, _b: cupy.ndarray, level: int = 0):
        return super().structure(self.down_sample(_a, level), self.down_sample(_b, level))

    def __call__(self, _a: cupy.ndarray, _b: cupy.ndarray):
        axis = tuple(range(-self._dim, 0))
        _l = [cupy.mean(self.luminance(_a, _b, level=self._levels - 1) ** self._α, axis=axis)]
        _c = [cupy.mean(self.contrasts(_a, _b, level=level), axis=axis) ** value for level, value in enumerate(self._β)]
        _s = [cupy.mean(self.structure(_a, _b, level=level), axis=axis) ** value for level, value in enumerate(self._γ)]
        # Products
...
class CwSSIM:
        ...
        while status:
            ...
            for item in _t:
                if isinstance(item, cupy.ndarray):
                    ...
                    _m.extend(item)
                    if len(item[0]) > 0 and not isinstance(item[0], cupy.ndarray):
                        status = True
            _t = _m
        return _t

    def amp(self, _a: cupy.ndarray, _b: cupy.ndarray):
        ...
        for _a_item, _b_item in zip(self._transform.forward(_a), self._transform.forward(_b)):
            ...
            counter, accumulator = counter + 1, accumulator + cupy.mean(result, axis=axis)
        return accumulator / counter

    def phi(self, _a: cupy.ndarray, _b: cupy.ndarray):
        counter, accumulator = 0, 0
        axis = tuple(range(-self._dim, 0))
        for _a_item, _b_item in zip(self._transform.forward(_a), self._transform.forward(_b)):
            statistics = self.statistics(_a_item, _b_item)
    ...
    def __call__(self, _a: cupy.ndarray, _b: cupy.ndarray):
        ...
        for _a_item, _b_item in zip(self.seq(self._transform.forward(_a)), self.seq(self._transform.forward(_b))):
            ...
            if denominator.size - cupy.count_nonzero(denominator) > 0:
                eps = cupy.finfo(denominator).eps
                location = cupy.where(denominator == 0)
                p_numerator[location] = p_numerator[location] + eps
                denominator[location] = denominator[location] + eps
            counter, accumulator = counter + 1, accumulator + cupy.mean(p_numerator / denominator, axis=axis)
        return accumulator / counter
